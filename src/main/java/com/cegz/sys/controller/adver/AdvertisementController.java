...
//					}
	 ...
	 * @date 2019年02月15日
	 */
	@RequiresPermissions(value = { "subscribeDesign/getSubscribeDesignlist" })
	...
	public ResultData getSubscribeDesignlist(HttpServletRequest request, String version, Integer curPage,
											 Integer pageSize, Byte type, Byte status, String name, String phone, String startTime, String endTime){
		...
		if(type == null){
			return serverAck.getEmptyData().setMessage("类型为空");
		}
		if(status == null){
		...
		try {
				//	查询不同类型广告的数量
				Long count = subscribeDesignService.countSubscribeDesignByStatus(type,status,name,phone,startTime,endTime);
				if(count == null || count <= 0){
					return serverAck.getEmptyData();
				}
				//分页查询广告预约列表
				int startPage = (curPage - 1) * pageSize;
				List<SubscribeDesign> listData = subscribeDesignService.listSubscribeDesignByStatus(type, status, startPage, pageSize, name, phone, startTime, endTime);
				...
				for (int i = 0; i < listData.size(); i++) {
					...
					//申请者电话
					view.setCreateUserPhone(design.getCreateUserId().getPhone());
					//审核者电话
					if(design.getUpdateUserId() != null){
						view.setUpdateUserPhone(design.getUpdateUserId().getPhone());
					}
					if(design.getUpdateTime() != null){
						view.setUpdateTime(sdf.format(design.getUpdateTime()));
					}
					view.setType(design.getType());
